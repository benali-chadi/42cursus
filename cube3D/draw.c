...
/*   Updated: 2020/02/14 23:30:10 by cbenali-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_img(void)
{
	...
	g_info.img_data = (int *)mlx_get_data_addr(g_info.img_ptr, &bpp,
			&size_line, &endian);
}
...
{
	g_sprite--;
	g_spt[g_sprite].y = (i + 0.5) * (float)TILE_SIZE;
	g_spt[g_sprite].x = (j + 0.5) * (float)TILE_SIZE;
	g_spt[g_sprite].distance = dist_p(g_spt[g_sprite].x, g_spt[g_sprite].y,
...
void	update_spr_dis(t_player player)
{
	...
	i = -1;
	while (++i < g_count)
		g_spt[i].distance = dist_p(g_spt[i].x, g_spt[i].y,
				player.x_p, player.y_p);
	i = -1;
	while (++i < g_count)
	{

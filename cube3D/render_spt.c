...
/*   Updated: 2020/02/15 00:06:56 by cbenali-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_spt(int x, int y, int sp_size, int k)
{
	...
	i = 0;
	if (!(g_spt[k].ptr = mlx_xpm_file_to_image(g_info.mlx_ptr,
					g_info.paths.sp, &g_spt[k].x_s, &g_spt[k].y_s)))
	...
	while (i++ < sp_size)
	{
		...
		while (j++ < sp_size)
		{
			color = g_spt[k].data[g_spt[k].x_s * (j *
					g_spt[k].y_s / sp_size) + (i * g_spt[k].x_s / sp_size)];
			if (color != 0)
...
void	ft_sprite(t_player player, int i)
{
	...
	y_inter = g_info.win_height / 2 - sp_size / 2 + g_drw_wll.t;
	x_inter = (sp_angle - player.direction * VAL) /
		FOV * g_info.win_width + (g_info.win_width / 2 - sp_size / 2);
